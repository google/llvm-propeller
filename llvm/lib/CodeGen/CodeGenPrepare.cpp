...
bool CodeGenPrepare::runOnFunction(Function &F) {
  ...
  if (TM->getBasicBlockSections() == llvm::BasicBlockSection::All)
    F.setBasicBlockSections(true);

  if (TM->getBasicBlockSections() == llvm::BasicBlockSection::Labels)
    F.setBasicBlockLabels(true);

  if (ProfileGuidedSectionPrefix) {
    if (PSI->isFunctionHotInCallGraph(&F, *BFI)) {
      if (TM->getBasicBlockSections() == llvm::BasicBlockSection::Hot ||
      ...
      if (TM->getBasicBlockSections() == llvm::BasicBlockSection::Likely)
        F.setBasicBlockSections(true);
    }
