...
void CfgBuilder::ApplyIntraCfgChanges(
  ...
  // Because path profiles are not intermittent (e.g. due to the limited LBR
  // stack depth), rerouting of some path weights missing the path predecessor
  // cannot be confidently determined. For these paths, we drop the weights
  // along their edges as if they were not sampled in the profile. This is to
  // make sure we don't leave any residual edge weights in the edges that are
  // rerouted to the clones.
  ...
  for (const PathNode *path_node : all_paths_to_drop) {
    for (const auto &[child_bb_index, child] : path_node->children()) {
      ...
      // affected edges here. Affected edges of other paths cannot overlap these
      // weights, because if they do, their path predecessor would have
      // overlapped with the edges along a path. This cannot happen because
      // we don't clone a path when its path predecessor is in the affected
      // edges of another path.
      ...
      CHECK_NE(edge, nullptr)
          << "No edge from block with index " << path_node->node_bb_index()
          << " to block with index" << child->node_bb_index() << " in function "
          << cfg_->GetPrimaryName().str()
          << " [fuenction index: " << cfg_->function_index() << "]";
