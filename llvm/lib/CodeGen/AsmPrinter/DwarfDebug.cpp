...
bool DwarfDebug::buildLocationList(SmallVectorImpl<DebugLocEntry> &DebugLoc,
  ...
  for (auto EB = Entries.begin(), EI = EB, EE = Entries.end(); EI != EE; ++EI) {
    ...
    if (std::next(EI) == Entries.end()) {
      ...
      EndLabel = Asm->MBBSectionRanges[EndMBB.getSectionID().toIndex()].EndLabel;
      if (EI->isClobber())
        EndMI = EI->getInstr();
    }
...
void DwarfDebug::emitDebugLocDWO() {
  ...
  for (const auto &List : DebugLocs.getLists()) {
    ...
    for (const auto &Entry : DebugLocs.getEntries(List)) {
      ...
      if (Asm->TM.getBBSectionsType() != llvm::BasicBlockSection::None &&
          Asm->TM.getBBSectionsType() != llvm::BasicBlockSection::Labels) {
        ...
        Asm->emitULEB128(idx);
        // Also the pre-standard encoding is slightly different, emitting this
        // as an address-length entry here, but its a ULEB128 in DWARFv5
        // loclists.
