...
MATCHER_P2(
    ...
    absl::StrCat(
        " is a path predecessor info that ",
        (negation ? " doesn't have" : " has"), " entries that ",
        DescribeMatcher<absl::flat_hash_map<int, propeller::PathPredInfoEntry>>(
            entries_matcher, negation),
        (negation ? " or doesn't have" : " and has"),
        " missing predecessor entry that ",
        DescribeMatcher<int>(missing_pred_entry_matcher, negation))) {
