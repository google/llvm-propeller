...
class HeapNode {
 public:
  ...
  HeapNode(K k, V v) : key(k), value(std::move(v)), parent(nullptr), children(2, nullptr){}

  HeapNode<K,V,CmpK,CmpV> * leftChild() {
    return children.at(0);
  }

  HeapNode<K,V,CmpK,CmpV> * rightChild() {
  ...
  void adoptChildren(const std::vector<HeapNode<K,V,CmpK,CmpV>*>& _children){
...
class Heap {
 private:
  ...
  HeapNode<K,V,CmpK,CmpV> * getNodeWithHandleHelper(HeapNode<K,V,CmpK,CmpV> * node, unsigned handle) {
    if(handle == 1)
      return node;
  ...
  void swapWithParent(HeapNode<K,V,CmpK,CmpV>* node) {
    ...
    if (par_old_left == node)
      node->adoptChildren(std::vector<HeapNode<K,V,CmpK,CmpV>*>({par, par_old_right}));
    else
      node->adoptChildren(std::vector<HeapNode<K,V,CmpK,CmpV>*>({par_old_left, par}));
  }
