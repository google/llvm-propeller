...
#ifndef PROPELLER_NODE_CHAIN_H_
...
class NodeChain {
 public:
  // Constructor for building a chain from a 2D vector of nodes, where each
  // inner vector of nodes must be placed in a bundle.
  explicit NodeChain(std::vector<std::vector<const CFGNode *>> nodes)
      : id_(nodes.front().front()->inter_cfg_id()),
        function_index_(nodes.front().front()->function_index()) {
    size_ = 0;
    freq_ = 0;
    for (auto &bundle_nodes : nodes) {
      node_bundles_.push_back(std::make_unique<CFGNodeBundle>(
          std::move(bundle_nodes), *this, node_bundles_.size(), size_));
      size_ += node_bundles_.back()->size();
      freq_ += node_bundles_.back()->freq();
    }
  }
  ...
  const InterCfgId &id() const { return id_; }
