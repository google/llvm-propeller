...
absl::StatusOr<CfgChangeFromPathCloning> GetCfgChangeForPathCloning(
  ...
  // This tracks paths with missing path predecessor at every block in the
  ...
  std::vector<const PathNode *> current_paths_with_missing_pred;
  ...
  for (int i = 0; i < path_from_root.size(); ++i) {
    ...
    for (const PathNode *&path_with_missing_pred :
         current_paths_with_missing_pred) {
      path_with_missing_pred =
          path_with_missing_pred->GetChild(path_node->node_bb_index());
      if (path_with_missing_pred == nullptr ||
          !path_with_missing_pred->path_pred_info().missing_pred_entry.freq) {
        continue;
      }
      cfg_change.paths_to_drop.push_back(path_with_missing_pred);
    }
    ...
    if (new_path_with_missing_pred != nullptr &&
      ...
      cfg_change.paths_to_drop.push_back(new_path_with_missing_pred);
      current_paths_with_missing_pred.push_back(new_path_with_missing_pred);
    }
